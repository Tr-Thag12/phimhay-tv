  ...
  app.innerHTML = `<div class="detail-page">
    ...
    <div class="container">
      ...
      <div class="tab-panel ${state.detailTab === 'episodes' ? 'active' : ''}">
        ...
        <div class="episode-grid">${seasonEpisodes.map(ep => `<a class="episode-card ${ep.id === state.currentEpisodeId ? 'active' : ''}" href="${watchUrl(movie, ep.id)}"><img src="${ep.thumb}" alt="Tập ${ep.number}" width="260" height="146" loading="lazy" decoding="async" ${imageFallbackAttr('backdrop')}><div><strong>Tập ${ep.number}: ${escapeHTML(ep.title)}</strong><p class="muted">${ep.duration}</p></div></a>`).join('')}</div>` : '<div class="empty-state"><span>' + icon('film') + '</span><strong>Phim lẻ không có danh sách tập.</strong></div>'}
      </div>
