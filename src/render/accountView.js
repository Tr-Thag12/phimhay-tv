...
export function renderAccount(app) {
  ...
  app.innerHTML = `<div class="account-page container">
    <div class="profile-head"><img src="https://images.unsplash.com/photo-1633332755192-727a05c4013d?q=80&w=160&auto=format&fit=crop" alt="Avatar" width="80" height="80" loading="lazy" decoding="async" ${imageFallbackAttr('avatar')}><div><h1>${escapeHTML(user.name)}</h1><p class="muted">${escapeHTML(user.email)} • ${user.plan}</p></div></div>
    <div class="account-layout">
      ...
      <section>
        ...
        <div class="account-panel ${state.accountTab === 'history' ? 'active' : ''}">${history.length ? history.map(item => `<div class="history-item" onclick="playMovie(${item.movie.id}, '${item.episodeId || ''}')"><img src="${item.movie.backdrop}" alt="${escapeHTML(item.movie.title)}" width="260" height="146" loading="lazy" decoding="async" ${imageFallbackAttr('backdrop')}><div style="flex:1"><strong>${escapeHTML(item.movie.title)}</strong><p class="muted">Đã xem ${item.progress}%</p><div class="bar"><span style="width:${item.progress}%"></span></div></div></div>`).join('') : `<div class="empty-state"><span>${icon('history')}</span><strong>Chưa có lịch sử xem.</strong></div>`}</div>
