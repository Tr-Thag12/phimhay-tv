...
Bước hiện tại chỉ tạo khung admin an toàn, chưa có CRUD admin thật. Backend có endpoint `GET /api/admin/health` được bảo vệ bằng `authMiddleware` và `requireAdmin`; frontend có route `/admin` để kiểm tra quyền và hiển thị admin shell cơ bản.
...
Các card “Quản lý phim”, “Quản lý tập phim”, “Quản lý thể loại”, “Quản lý người dùng”, “Quản lý bình luận/báo lỗi” chỉ là placeholder. CRUD admin sẽ được phát triển ở bước sau.
...
- Backend hiện có health check, Docker Compose PostgreSQL local, Prisma migration đầu tiên, seed dữ liệu mẫu, API public, Auth API cơ bản và API `/api/me` cho watchlist/history trong `server/`. Frontend đã nối API public, Auth API cơ bản và đồng bộ watchlist/history theo tài khoản khi chạy fullstack local, nhưng chưa có admin CRUD.
