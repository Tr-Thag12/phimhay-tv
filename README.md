...
Admin Movie API mới chỉ có backend. Frontend admin shell vẫn chưa có form CRUD quản lý phim.
...
Bước hiện tại có khung admin an toàn và backend Admin Movie API cơ bản. Backend có endpoint `GET /api/admin/health` và nhóm `GET/POST/PATCH/DELETE /api/admin/movies`, đều được bảo vệ bằng `authMiddleware` và `requireAdmin`; frontend có route `/admin` để kiểm tra quyền và hiển thị admin shell cơ bản.
...
3. Vào `/admin`; nếu backend đang chạy và token là ADMIN, trang hiển thị “Quản trị PhimHay TV” cùng các card placeholder.
...
Các card “Quản lý phim”, “Quản lý tập phim”, “Quản lý thể loại”, “Quản lý người dùng”, “Quản lý bình luận/báo lỗi” chỉ là placeholder. Backend đã có Admin Movie API, nhưng CRUD UI trên frontend sẽ được phát triển ở bước sau.
...
- Backend hiện có health check, Docker Compose PostgreSQL local, Prisma migration đầu tiên, seed dữ liệu mẫu, API public, Auth API cơ bản, API `/api/me` cho watchlist/history và Admin Movie API backend trong `server/`. Frontend đã nối API public, Auth API cơ bản và đồng bộ watchlist/history theo tài khoản khi chạy fullstack local, nhưng chưa có frontend admin CRUD.
