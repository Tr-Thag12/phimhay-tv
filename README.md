...
Đây là base project cho website xem phim PhimHay TV. Frontend đang chạy bằng Vite, HTML/CSS/JavaScript thuần. Backend hiện có Node.js + Express + Prisma trong thư mục `server/`, kèm PostgreSQL local bằng Docker Compose, migration đầu tiên, seed dữ liệu mẫu, API public cho phim/thể loại/tìm kiếm, Auth API cơ bản bằng JWT, API thư viện người dùng cho watchlist/history và Admin Movie API. Frontend hiện đã gọi API public, auth, thư viện người dùng và có giao diện admin quản lý phim nối trực tiếp với Admin Movie API; khi backend public không khả dụng, trang phim vẫn fallback về dữ liệu mock.
...
## Admin shell local và quản lý phim

Bước hiện tại có khung admin an toàn và giao diện quản lý phim thật. Backend có endpoint `GET /api/admin/health` và nhóm `GET/POST/PATCH/DELETE /api/admin/movies`, đều được bảo vệ bằng `authMiddleware` và `requireAdmin`; frontend có route `/admin` để kiểm tra quyền, hiển thị admin shell và render tab `Quản lý phim`.
...
3. Vào `/admin`; nếu backend đang chạy và token là ADMIN, trang hiển thị “Quản trị PhimHay TV” cùng tab `Quản lý phim`.
...
Trong tab `Quản lý phim`, ADMIN có thể tải lại danh sách, tìm kiếm/lọc/phân trang, xem chi tiết, tạo phim, sửa phim, bật/tắt publish, bật/tắt featured và xóa mềm phim. Xóa mềm chỉ ẩn phim khỏi public bằng `isPublished=false`, `isFeatured=false`, `status=HIDDEN`; không hard delete. Các nhóm “Quản lý tập phim”, “Quản lý thể loại”, “Quản lý người dùng”, “Quản lý bình luận/báo lỗi” vẫn chưa làm trong bước này.

Form quản lý phim chưa upload file thật. Poster, backdrop và trailer hiện nhập bằng URL; `countryId` có thể nhập thủ công từ dữ liệu hiện có, còn `categoryIds` nhập dạng danh sách id cách nhau bằng dấu phẩy.
...
- Admin `/admin` có giao diện quản lý phim nối với Admin Movie API cho ADMIN
...
- Backend hiện có health check, Docker Compose PostgreSQL local, Prisma migration đầu tiên, seed dữ liệu mẫu, API public, Auth API cơ bản, API `/api/me` cho watchlist/history và Admin Movie API backend trong `server/`. Frontend đã nối API public, Auth API cơ bản, đồng bộ watchlist/history theo tài khoản và frontend admin CRUD phim khi chạy fullstack local.
